    ...
    @foreach($reviews as $review)
        <div class="flex w-full p-4 bg-gray-100 rounded-lg mb-3">
            ...
            <div class="flex-auto ml-2">
                <div class="flex flex-row">
                    ...
                    <div class="ml-2">
                        @for($i = 1; $i <= $review->rate; $i++)
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-yellow-400 inline-block" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                            </svg>
                        @endfor
                        @for($i = 1; $i <= (5 - $review->rate); $i++)
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-300 inline-block" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                            </svg>
                        @endfor
                    </div>
                ...
                <div class="flex flex-row">
                    <div class="flex-auto">
                        <span class="text-sm text-gray-400 font-medium">{{ $review->likes ? $review->likes : '0' }} likes</span>
                        <span class="text-sm text-gray-400 font-medium ml-2">{{ $review->unlikes ? $review->unlikes : '0' }} unlikes</span>
                        <span class="text-sm text-gray-400 font-medium ml-2">{{ $review->shares ? $review->shares : '0' }} shares</span>
                    </div>
                    {{-- <div class="flex-none" >
